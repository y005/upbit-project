...
public class UpbitCrawlerService {
  ...
  @CrawlErrorHandler
  public void saveCoin5MinCandleInfo(MarketType marketType, int count,
      LocalDateTime localDateTime) {
    List<MinuteCandle> result = upbitCrawlClient.getMinuteCandle(MinuteType.FIVE, marketType, count,
        localDateTime);
    result.stream()
        .map((e) -> new Btc5MinuteCandle(e.getCandleDateTimeKst(), e.getOpeningPrice(),
            e.getTradePrice(), e.getHighPrice(), e.getLowPrice(), e.getCandleAccTradeVolume()))
        .forEach(btc5MinuteCandleRepository::save);

  }

  @CrawlErrorHandler
  public void saveCoin5MinCandleInfoBefore1Hour(MarketType marketType,
      LocalDateTime localDateTime) {
    LocalDateTime before1Hour = localDateTime.minusHours(1);
    List<MinuteCandle> result = upbitCrawlClient.getMinuteCandle(MinuteType.FIVE, marketType, 12,
        before1Hour);
    result.stream()
        .map((e) -> new Btc5MinuteCandle(e.getCandleDateTimeKst(), e.getOpeningPrice(),
            e.getTradePrice(), e.getHighPrice(), e.getLowPrice(), e.getCandleAccTradeVolume()))
  ...
  public MinuteCandle getRecent5MinCandleInfo(MarketType marketType) {
    return upbitCrawlClient.getMinuteCandle(MinuteType.FIVE, marketType, 1, LocalDateTime.now())
        .get(0);
  }
}
